...
$(document).ready(function() {
   ...
   var array=[{
    ...
    start: '2020-08-24T09:30',
    end: '2020-08-24T10:00',
    type: 'Class Event',
    calendar: '1st Year',
    textColor: "#ffffff",
}]
db.collection("calendar").doc(roomID).get().then(function(doc) {
...
//$('#calendar').fullCalendar('addEventSource',array);
  ...
  newEvent = function(start, end, eventType) {
      ...
      if (title) {
        ...
        db.collection('calendar').doc(roomID).set({[ eventData._id]:{id:eventData._id,title:eventData.title,start:eventData.start,end:eventData.end,description:eventData.description,
          type:eventData.type,calendar:eventData.calendar,background:eventData.backgroundColor,text:eventData.textColor,}
  ...
  //EDIT EVENT CALENDAR
    ...
    editEvent = function(event, element, view) {
        ...
        $('#updateEvent').on('click', function() {
          ...
          if (title) {
            ...
            console.log(event._id);
                  ...
                  db.collection('calendar').doc(roomID).update({[event.id]:{
                    [event._id]:{id:event._id,title:event.title,start:event.start,end:event.end,description:event.description,
                      type:event.type,calendar:event.calendar,background:event.backgroundColor,text:event.textColor,}}
                  });
          } 
          else {
          alert("Title can't be blank. Please try again.")
        ...
        $('#deleteEvent').on('click', function() {
         ...
         db.collection('calendar').doc(roomID).update({
